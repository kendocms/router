...
namespace Kendocms\Router;

/**
 * Kendocms Router [Core]
 * ---------------
...
class Core
{
    ...
    /**
     * Force call the Kendocms URL handler
     *
     ...
     * The argument must be an instance that subclasses Kendocms_Middleware.
     *
     * @param \Kendocms\Middleware $newMiddleware The middleware to set
     */
    ...
    /**
     * Register a middleware in your Kendocms application by name
     * 
    ...
    public static function getRoute(): array
    {
        ...
        if (isset($currentRoute[0])) {
            $route = array_merge($route, [
                ...
                'method' => \Leafs\Http\Request::getMethod(),
    ...
    public static function run(?callable $callback = null)
    {
        ...
        if (class_exists('Kendocms\App')) {
            $config = array_merge($config, [
                'mode' => \Kendocms\Config::get('mode') ?? 'development',
                'app.down' => \Kendocms\Anchor::toBool(\Kendocms\Config::get('app.down')) ?? false,
                'debug' => \Kendocms\Anchor::toBool(\Kendocms\Config::get('debug')) ?? false,
            ]);
        }
            ...
            if (!static::$downHandler) {
                if (class_exists('Kendocms\App')) {
                    static::$downHandler = function () {
                        \Kendocms\Exception\General::defaultDown();
                    };
                } else {
                    static::$downHandler = function () {
        ...
        static::$requestedMethod = \Leafs\Http\Request::getMethod();
            ...
            if (!static::$notFoundHandler) {
                if (class_exists('Kendocms\App')) {
                    static::$notFoundHandler = function () {
                        \Kendocms\Exception\General::default404();
                    };
                } else {
                    static::$notFoundHandler = function () {
